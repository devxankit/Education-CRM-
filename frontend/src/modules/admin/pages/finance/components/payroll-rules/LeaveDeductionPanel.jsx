...
const LeaveDeductionPanel = ({ isLocked }) => {

    const [rules, setRules] = useState({
        lopFormula: 'fixed_30', // fixed_30 | actual_days
        sandwichRule: false,
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                    ...
                    <div className="space-y-3">
                        <label className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-all ${rules.lopFormula === 'fixed_30' ? 'bg-indigo-50 border-indigo-200 ring-1 ring-indigo-500' : 'bg-white border-gray-200 hover:bg-gray-50'}`}>
                            <input
                                type="radio"
                                name="lop"
                                checked={rules.lopFormula === 'fixed_30'}
                        ...
                        <label className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-all ${rules.lopFormula === 'actual_days' ? 'bg-indigo-50 border-indigo-200 ring-1 ring-indigo-500' : 'bg-white border-gray-200 hover:bg-gray-50'}`}>
                            <input
                                type="radio"
                                name="lop"
                                checked={rules.lopFormula === 'actual_days'}
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 space-y-4">
                        <div className="flex items-start gap-3">
                            <input
                                ...
                                checked={rules.sandwichRule}
                                onChange={(e) => handleChange('sandwichRule', e.target.checked)}
                                disabled={isLocked}
                                className="mt-1 w-4 h-4 text-indigo-600 rounded"
                            />
                            <div>
                                <label htmlFor="sandwich" className="block text-sm font-medium text-gray-900">Enable "Sandwich Rule"</label>
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            />
                            <div>
                                <label htmlFor="weekends" className="block text-sm font-medium text-gray-900">Include Weekends in Continuous Leave</label>
                                <p className="text-xs text-gray-500 leading-relaxed">
                    ...
                    <div className="flex items-center gap-2 text-[10px] text-gray-400 bg-white p-2 rounded border border-gray-100">
                        <AlertCircle size={12} />
                        <span>Changes to these policies will affect payroll calculation for the CURRENT month onwards.</span>
                    </div>
