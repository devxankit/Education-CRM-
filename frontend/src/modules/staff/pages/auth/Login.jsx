...
const StaffLogin = () => {
    ...
    // Open forgot-password flow when user clicks reset link in email (?forgot=1)
    React.useEffect(() => {
    ...
    const handleSendOtp = async (e) => {
        ...
        setIsLoading(true);
        const result = await requestStaffPasswordReset(forgotEmail);
        setIsLoading(false);
        if (result.success) {
    ...
    const handleResetPassword = async (e) => {
        ...
        setIsLoading(true);
        const result = await resetStaffPasswordWithOtp(forgotEmail, forgotOtp, forgotPassword);
        setIsLoading(false);
        if (result.success) {
            setSuccessMsg(result.message || 'Password reset successfully. You can now login.');
            setForgotStep(4);
        } else {
        ...
        <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans">
            <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
                ...
                <p className="mt-2 text-sm text-gray-600">
                    {showForgot ? 'Reset your password' : 'Secure access for authorized personnel only'}
                </p>
            </div>
                            ...
                            {forgotStep === 1 && (
                                <form className="space-y-6" onSubmit={handleSendOtp}>
                                    <div>
                                        ...
                                        />
                                        <p className="mt-1 text-xs text-gray-500">We will send an OTP to this email.</p>
                                    </div>
                                ...
                                <form className="space-y-6" onSubmit={handleResetPassword}>
                                    {successMsg && (
                                        <div className="rounded-md bg-blue-50 p-4 border border-blue-100">
                                            <p className="text-sm font-medium text-blue-800">{successMsg}</p>
                                        </div>
                                    )}
                                    <div>
