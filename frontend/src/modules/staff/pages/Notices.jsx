...
const Notices = () => {
    ...
    const fetchNotices = useStaffStore(state => state.fetchNotices);
    ...
    const filteredNotices = notices.filter(notice => {
        ...
        const matchesPriority = filterPriority === 'All' || notice.priority === filterPriority.toUpperCase();
            ...
            <div className="p-4 space-y-3">
                ...
                {!loading && filteredNotices.map(notice => (
                    <div
                        ...
                        className={`bg-white p-4 rounded-xl border shadow-sm cursor-pointer hover:shadow-md transition-all active:scale-[0.99] group border-gray-200`}
                    >
                        <div className="flex justify-between items-start mb-2">
                            ...
                            <span className="text-[10px] text-gray-400 font-medium">{new Date(notice.publishDate).toLocaleDateString()}</span>
                        </div>

                        <h3 className={`text-sm font-medium text-gray-700 mb-2 group-hover:text-indigo-600 transition-colors`}>
                            {notice.title}
                        </h3>

                        <div className="flex items-center justify-between pt-2 border-t border-gray-50 mt-2">
                            <span className="text-[10px] text-gray-400 font-mono">{notice.noticeId}</span>
                        </div>
                    </div>
                ))}
